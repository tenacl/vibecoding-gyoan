...
| 13:00–14:50 | 프로젝트 | 프로젝트 4 · 주식 시세 대시보드 |
...
> (도식: 주식시세 API 대시보드 구조 도식)

---

## 주식 시세 대시보드 — 핵심 내용

- 오늘 쓸 API = **금융위원회_주식시세정보** (data.go.kr) — **자동승인**, 개발계정 1만 건/일
- 한 번 호출로 **시가·고가·저가·종가·거래량·시가총액**이 다 온다 — 차트 재료가 응답 안에 있다
- **API 문서를 `@`로 첨부** — `@주식시세-api-문서.txt`. 문서를 안 주면 Claude가 파라미터를 **추측**한다
- 활용가이드는 상세 페이지에서 내려받아 프로젝트 폴더에 (`.docx` → 파라미터 표를 복사해 `.txt`로 저장)
- **시각화** = JSON을 그대로 뿌리는 게 아니라 필요한 숫자만 골라 **차트로** — "종목별 종가를 막대차트로 그려줘"
- **일별 시세**(실시간 아님) — 기준일자는 **직전 거래일**로, 주말·공휴일은 0건
- 어제·오늘 배운 게 조립 — 프론트엔드(Day1) + API(오전)
- ⚠️ **학습용 시각화 연습** — 투자 판단 근거가 아니다

---

## 단계별 체크포인트 — 순서대로 하나씩

1. API 활용가이드 문서를 **`@`로 첨부**했다 — 여기서 시작
2. API 응답(JSON)이 F12 콘솔에 찍힌다 — 여기까지가 절반
3. 관심 종목 3~5개의 **종가가 막대차트**로 그려진다
4. 거래량 차트를 나란히 놓고 제목·단위·기준일자를 붙여 **대시보드 화면**이 된다

**EXAMPLE PROMPT (시작)**
```
공공데이터포털의 '금융위원회_주식시세정보' API를 쓰는 주식 대시보드 웹페이지를 만들자.
API 규격은 @주식시세-api-문서.txt 를 참고해서 그대로 따라줘. 없는 파라미터는 지어내지 마.
단계별로 가자. 먼저 기준일자 하나로 API를 호출해서 응답이 브라우저 콘솔에 찍히는 것까지만 해줘.
인증키는 발급받아 뒀어. 넣을 위치를 알려줘.
```

> 💡 `@파일이름` = "이 파일을 읽고 해". 말로 설명하는 것보다 **파일째로 건네는 쪽이 항상 정확하다**
...
| 증상 | 해결 |
| --- | --- |
| Claude가 **없는 파라미터를 지어냄** | 문서를 안 본 것 — "@주식시세-api-문서.txt를 다시 읽고 파라미터를 문서대로 고쳐줘" |
| "등록되지 않은 키" 에러 | 일반 키와 **인코딩 키 2가지** — "인코딩 키로 바꿔서 다시 호출해줘" |
| 에러 없는데 **결과 0건** | 기준일자가 **주말·공휴일** — "기준일자를 직전 거래일로 자동 계산하게 고쳐줘" |
| 한글 종목명 넣으면 결과 없음 | URL 인코딩이 한 번 더 필요 — "종목명 한글 파라미터가 URL 인코딩되도록 고쳐줘" |
| 콘솔에 **CORS** 빨간 에러 | 브라우저가 외부 요청 막은 것(내 잘못 아님) — "CORS 우회 방법 적용해줘" |
| 에러 없는데 데이터 안 옴 | F12 → Network 탭 빨간 줄 캡처 → "요청은 가는데 데이터가 안 와. 원인 찾아줘" |

**이 장을 마치면** — [ ] 문서를 `@`로 첨부했다 [ ] 콘솔에서 JSON 봤다 [ ] 종가 막대차트 [ ] 제목·단위·기준일자 붙은 대시보드
...
- [ ] 공공데이터포털(data.go.kr) 가입 + **금융위원회_주식시세정보** 활용신청 미리 (자동승인) + **활용가이드 문서 내려받기**
