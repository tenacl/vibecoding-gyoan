...
2. 관심 종목 3~5개의 **종가가 막대차트**로 그려진다
3. 거래량 차트를 나란히 놓고 제목·단위·기준일자를 붙여 **대시보드 화면**이 된다
...
**이 장을 마치면** — [ ] 콘솔에서 JSON 봤다 [ ] 종가 막대차트 [ ] 제목·단위·기준일자 붙은 대시보드
...
- [ ] 공공데이터포털(data.go.kr) 가입 + **금융위원회_주식시세정보** 활용신청 미리 (자동승인)
